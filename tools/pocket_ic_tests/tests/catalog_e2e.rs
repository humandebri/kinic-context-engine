...
async fn catalog_canister_deploys_and_resolves_fixture_sources() -> Result<()> {
...
#[tokio::test]
#[ignore = "requires POCKET_IC_BIN=/path/to/pocket-ic-server"]
async fn engine_query_and_pack_work_against_pocket_ic() -> Result<()> {
    ensure_pocket_ic_server()?;
...
#[tokio::test]
#[ignore = "requires POCKET_IC_BIN=/path/to/pocket-ic-server"]
async fn engine_query_and_pack_error_contracts_stay_stable() -> Result<()> {
    ensure_pocket_ic_server()?;
